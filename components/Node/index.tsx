...
export default function TextNode({ data, id }: { data: NodeData; id: string }) {
  ...
  return (
    <div className="text-sm min-w-64 max-w-80 pb-4 rounded-xl bg-white border border-zinc-200 backdrop-blur-sm bg-[rgba(255, 255, 255, 0.5)] shadow-md">
      <div>
        <div
          className={`flex justify-between w-full bg-zinc-100 p-1 pl-2 pr-2 border-b border-zinc-200 rounded-t-xl  custom-drag-handle`}
        >
          ...
          <div>
            <MinusIcon className={`h-4 w-4 animate-spin ${!loading && 'invisible'}`} />
          </div>
        </div>
        <div className="p-2">
          <Content content={content} fold={fold} />
        </div>
        <Tool id={id} concepts={concepts} content={content} setLoading={setLoading}/>
      </div>
